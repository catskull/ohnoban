...
function loadBoard(data) {
  ...
  function serializeBoard() {
    const columns = [...document.querySelectorAll(".column")].map(col => ({
      ...
      title: col.querySelector("h2").childNodes[0].textContent,
      cards: [...col.querySelectorAll(".card")].map(li => readCard(li))
    }));
    return { title: data.title, tags: { ...data.tags }, columns };
  }

  function save() {
    localStorage.setItem("ohnoban-board", JSON.stringify(serializeBoard()));
  }
  ...
  function renderColumn(col) {
    ...
    header.innerHTML = `${esc(col.title)}<span class="count">${col.cards.length}</span>`;

    const colActions = document.createElement("span");
    colActions.className = "col-actions";
    colActions.innerHTML = `<button class="col-move-btn" data-dir="left" title="Move left">\u2039</button><button class="col-move-btn" data-dir="right" title="Move right">\u203a</button><button class="col-delete-btn" title="Delete column">\u00d7</button>`;
    colActions.querySelector(".col-delete-btn").addEventListener("click", () => {
      const cards = section.querySelectorAll(".card").length;
      if (cards > 0 && !confirm(`Delete "${col.title}" and its ${cards} card${cards > 1 ? "s" : ""}?`)) return;
      section.remove();
      save();
    });
    colActions.addEventListener("click", e => {
      ...
      if (!btn) return;
      const sibling = btn.dataset.dir === "left" ? section.previousElementSibling : section.nextElementSibling;
      if (!sibling || !sibling.classList.contains("column")) return;
  ...
  function rebuildTagCheckboxes() {
    tagOptions.innerHTML = "<legend>Tags</legend>";
  ...
  function populateTagManager() {
    tagList.innerHTML = "";
    for (const [name, color] of Object.entries(data.tags)) {
      tagList.appendChild(createTagRow(name, color));
    }
  }
